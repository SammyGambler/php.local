...
function isValidQuestion($qText, $uMail, $uName, &$errMsg = null)
{
    $errMsg = '';
    $hasError = false;
    if (strlen($qText) < QA_QUSETION_MIN_LENGTH) {
        $errMsg .= "سوال کوتاه است.\n";
        $hasError = true;
    }
    if (!filter_var($uMail, FILTER_VALIDATE_EMAIL)) {
        $errMsg .= "ایمیل وارد شده نامعتبر است.\n";
        $hasError = true;
    }
    if (strlen($uName) < QA_UNAME_MIN_LENGTH) {
        $errMsg .= "نام وارد شده کوتاه است.\n";
        $hasError = true;
    }
    if ($hasError) {
        return false;
...
/***** Database Functions *****/
function addQuestion($uName, $uMail, $uMobile, $qText, &$errorMsg = '')
{
    global $db;
    // sanitize all inputs in one line !!!
    list($uName, $uMail, $uMobile, $qText) = array(sanitize($uName), sanitize($uMail), sanitize($uMobile), sanitize($qText));
    $sql = "INSERT INTO $db->questionTable (uname, umail, umobile, text) VALUES ('$uName', '$uMail', '$uMobile', '$qText');";
    $result = $db->query($sql);
    if ($result) {
        return true;
    }
...
function getAnswers($qid)
{
    ...
    if ($result) {
        ...
        foreach ($answers as $a) {
            $str .= '<div class="a" id="a-' . $a['id'] . '">' . nl2br($a['text']) . '<span class="date">(' . jdate(QA_DATE_FORMAT, strtotime($a['create_date'])) . ')</span></div>' . PHP_EOL;
        }
        return $str;
    }
    return '';
